...
class QUnFrameWindow(QMainWindow):
    ...
    def homepage3(self):
        pdd = 1
        if self.tt == '快速检测':
            if self.page2.edt.text() == '' or self.page2.local_edt.text() == '':
                QMessageBox.information(self, "提示", "未选择数据集")
                pdd = 2
            if os.path.exists(self.page2.local_edt.text()) == False:
                QMessageBox.information(self, "提示", "路径错误")
                pdd = 2
            if self.page2.edt.text() not in self.page2.result:
                QMessageBox.information(self, "提示", "对比数据选择错误")
                pdd = 2
        elif self.tt == '慢速检测':
            if self.page10.edt.text() == '' or self.page10.local_edt.text() == '':
                QMessageBox.information(self, "提示", "未选择数据集")
                pdd = 2
            if os.path.exists(self.page10.local_edt.text()) == False:
                QMessageBox.information(self, "提示", "路径错误")
                pdd = 2
            if self.page10.edt.text() not in self.page10.result:
                QMessageBox.information(self, "提示", "对比数据选择错误")
                pdd = 2
        elif self.tt == '本地检测':
            if self.page11.local_edt.text() == '':
                QMessageBox.information(self, "提示", "未选择数据集")
                pdd = 2
            if os.path.exists(self.page11.local_edt.text()) == False:
                QMessageBox.information(self, "提示", "路径错误")
                pdd = 2
        if pdd==1:
            ...
            self.page8.edt5.setText(tim1)
            self.page8.edt6.setText(txt1)
            self.page8.buttn2.clicked.connect(self.homepage)

            abb = {"time": f"{tim1}", "content": f"{self.page8.edt1.text()}与{self.page8.edt2.text()}对比结果：{txt1}", "type": "污染检测"}
