...
class setpage(QWidget):
    ...
    def initUI(self):
        ...
        self.back_btn.clicked.connect(self.feedback)
    ...
    def cheaknew(self):
        QMessageBox.information(self, "更新检查",
                                "当前版本已为最新版本")  # 最后的Yes表示弹框的按钮显示为Yes，默认按钮显示为OK,不填QMessageBox.Yes即为默认
    ...
    def restoreconfig(self):
        ...
        abb = {"time": f"{tim1}", "content": "设置重置", "type": "设置操作"}
    ...
    def feedback(self):
        feedinput = QInputDialog.getText(self, "反馈", "输入反馈信息")
        if feedinput[0] != '':
            QMessageBox.information(self, "提示", "感谢您的反馈")
