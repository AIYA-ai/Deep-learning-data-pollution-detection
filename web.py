...
from PyQt5.QtWidgets import QWidget, QTabWidget, QApplication, QLineEdit, QPushButton, \
    QHBoxLayout, QVBoxLayout, QListWidgetItem, QListWidget, QMessageBox, QLabel, QCheckBox
...
class WebEngineView(QWebEngineView):
    ...
    def on_downloadRequested(self, downloadItem):
        if downloadItem.isFinished() == False and downloadItem.state() == 0:
            ...
            abb = {"time": f"{tim1}", "content": f"{the_filename}", "type": "数据下载"}
...
class Webpage(QWidget):
    ...
    def initUI(self):
        ...
        self.btn8.clicked.connect(self.xxx)
...
class NewWindow1(QWidget):
    ...
    def initUI(self):
        self.hhbox = QHBoxLayout(self)
        self.titel = QLabel('开启黑名单检测')
        self.cheakbox = QCheckBox()
        self.cheakbox.setChecked(True)
        self.hhbox.addWidget(self.titel)
        self.hhbox.addWidget(self.cheakbox)
